...
const cartSlice = createSlice({
    ...
    reducers: {
        ...
        decreaseItemQuantity(state, action) {
            ...
            item.totalPrice = item.quantity * item.unitPrice;
            if (item.quantity === 0) {
                // here when the quantity is zero , we want to delete the item , sowe use the deleteItem reducer that we define before              
...
// current quantity
export const getCurrnetQuantityById = id => state => state.cart.cart.find(item => item.pizzaId === id)?.quantity ?? 0; 